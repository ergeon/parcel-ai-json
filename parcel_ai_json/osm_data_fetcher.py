...
class OSMDataFetcher:
    ...
    def _calculate_bbox(
        ...
        north_lon, north_lat, _ = geod.fwd(
            center_lon, north, 0,  # bearing 0° = north
            self.buffer_meters
        )

        # Buffer south
        south_lon, south_lat, _ = geod.fwd(
            center_lon, south, 180,  # bearing 180° = south
            self.buffer_meters
        )

        # Buffer east
        east_lon, east_lat, _ = geod.fwd(
            east, center_lat, 90,  # bearing 90° = east
            self.buffer_meters
        )

        # Buffer west
        west_lon, west_lat, _ = geod.fwd(
            west, center_lat, 270,  # bearing 270° = west
            self.buffer_meters
        )
    ...
    def _parse_building_element(
        self,
        element: Dict
    ) -> Optional[OSMBuilding]:
        ...
        try:
            ...
            if element['type'] == 'way':
                ...
                outer_ways = [
                    member for member in element.get('members', [])
