...
class TransformerDecoder(nn.Module):
    def __init__(
        ...
        if boxRPB != "none":
            ...
            if resolution is not None and stride is not None:
                ...
                # Use CPU for initialization; cache will be recreated on correct device at runtime
                device = "cpu"
    ...
    def _get_rpb_matrix(self, reference_boxes, feat_size):
        ...
        # Check if dynamo is available (PyTorch 2.5+)
        is_compiling = hasattr(torch.compiler, 'is_dynamo_compiling') and torch.compiler.is_dynamo_compiling()
        ...
        # Ensure coords are on the same device as reference_boxes (GPU fix for CUDA inference)
        # Cached coords might be on CPU from previous runs
        ...
        if not (hasattr(torch.compiler, 'is_dynamo_compiling') and torch.compiler.is_dynamo_compiling()):
